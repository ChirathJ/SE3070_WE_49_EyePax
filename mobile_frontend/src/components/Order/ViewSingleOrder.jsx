...
export default function ViewSingleOrder({ navigation, route }) {
  ...
  async function getOrder() {
    try {
      await axios
        .get(`http://192.168.1.5:8000/order/getById/${id}`)
        .then((res) => {
          if (res.status === 200) {
  ...
  return (
    <View>
      ...
      <ScrollView style={{ height: "61%", marginBottom: 3 }}>
          ...
          return (
            <Card key={id}>
              ...
              <Card.Image
                ...
                source={{
                  uri: `http://192.168.1.5:8000/routes/ProductManagement/ProductImages/${element.ProductImage}`,
                }}
              />
              <Text
                style={{
