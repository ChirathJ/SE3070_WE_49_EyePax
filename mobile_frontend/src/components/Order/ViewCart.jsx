...
function ViewCart({ navigation }) {
  ...
  async function getAllData() {
    try {
      await axios
        .get(`http://172.28.19.253:8000/cart/getAll/${userId}`)
        .then((res) => {
          if (res.status === 200) {
  ...
  async function handleConfirmOrder() {
    try {
      ...
      if (siteAddress === "") {
        ...
        await axios
          .post(`http://172.28.19.253:8000/order/add`, orderObject)
          .then((res) => {
            if (res.status === 201) {
  ...
  async function handleDeleteCart() {
    try {
      await axios
        .delete(`http://172.28.19.253:8000/cart/delete/${userId}`)
        .then((res) => {
          if (res.status === 200) {
            setSiteAddress("");
            getAllData();
  ...
  async function removeItem(id) {
    try {
      await axios
        .delete(`http://172.28.19.253:8000/cart/deleteOne/${id}`)
        .then((res) => {
          if (res.status === 200) {
            getAllData();
            alert("Item Deleted!");
    ...
    <View>
          ...
          return (
            <Card key={id} style={{ width: "100%", height: 80 }}>
              ...
              <Card.Image
                style={styles.image}
                source={{
                  uri: `http://172.28.19.253:8000/fetchImage/${element.ProductImage}`,
                }}
              />
              <Text
                style={{
      ...
      <View style={styles.row}>
        ...
        <View>
          ...
          <TouchableOpacity onPress={handleDateOpen.bind}>
